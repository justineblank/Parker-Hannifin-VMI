...
class CMyApplication
{
  ...
  public:
    ...
    void
    processTags(string filename, string jsonObject);

    void 
    getMacAddress(char * uc_Mac);

    int 
    enableGpo(int state);

    string m_ReaderHostName;
...
string macAddress;
bool supportLight = true;
bool flashTheLight = false;

/** Connection to the LLRP reader */
CConnection *               m_pConnectionToReader = NULL;
int                         m_Verbose = 0;
...
// Function to call command-line application and return results
...
// Writes tag data to disk and toggles the light on/off
void
CMyApplication::processTags(string filename, string jsonObject)
{
  enableGpo(0);
...
// Gets the MAC address for eth0
void CMyApplication::getMacAddress(char * uc_Mac)
{
  int fd;
  ...
  //display mac address
  macAddress = sprintf((char *)uc_Mac,(const char *)"%.2X:%.2X:%.2X:%.2X:%.2X:%.2X" , mac[0]&0xFF, mac[1]&0xFF, mac[2]&0xFF, mac[3]&0xFF, mac[4]&0xFF, mac[5]&0xFF);
}
  ...
  char *                        av[])
{
    ...
    myApp.getMacAddress (mac);
    macAddress = string(mac);
    cout << endl << "Mac Address : " << macAddress << endl;

    cout << "Starting reader application..." << endl;
 ...
 *****************************************************************************
 **
 ** @brief  Reads configuration parameters from config.json
 **
 **
 *****************************************************************************/
void
CMyApplication::loadConfigFile()
{
  ...
  if(document.HasMember("TxPower"))
  {
    if(document["TxPower"].IsNumber())
    {
      cout << "Found Tx Power Setting: " << document["TxPower"].GetInt() << endl;
      m_apiUrl = document["TxPower"].GetInt();
    }
    else
    {
      cout << "Tx Power Setting is not an integer." << endl;
    }
  }
  else
  {
    cout << "No Tx Power Setting was in the config file" << endl;
  }

  if(document.HasMember("RxSensativity"))
  {
    if(document["RxSensativity"].IsNumber())
    {
      cout << "Found Rx Sensativity Setting: " << document["RxSensativity"].GetInt() << endl;
      m_apiUrl = document["RxSensativity"].GetInt();
    }
    else
    {
      cout << "Rx Sensativity Setting is not an integer." << endl;
    }
  }
  else
  {
    cout << "No Rx Sensativity Setting was in the config file" << endl;
  }

  if(document.HasMember("Antennas"))
  {
    if(document["Antennas"].IsNumber())
    {
      cout << "Found Antennas Setting: " << document["Antennas"].GetInt() << endl;
      m_antennas = document["Antennas"].GetInt();
    }
    else
    {
      cout << "Antennas Setting is not an integer." << endl;
    }
  }
  else
  {
    cout << "No Antennas Setting was in the config file" << endl;
  }
}
...
int 
CMyApplication::enableGpo(int state)
{
    CSET_READER_CONFIG *pCmd;
    CMessage *pRspMsg;
    CSET_READER_CONFIG_RESPONSE *pRsp;
    ...
    // Enable the GPO port
    CGPOWriteData *pGpoPortCurrentState = new CGPOWriteData();
    pGpoPortCurrentState->setGPOPortNumber(1);
    pGpoPortCurrentState->setGPOData(state);

    pCmd->addGPOWriteData(pGpoPortCurrentState);
...
//        pImpIsm->setInventorySearchMode(ImpinjInventorySearchType_Dual_Target);
        pImpIsm->setInventorySearchMode(ImpinjInventorySearchType_Single_Target_With_Suppression);
        ...
        {
            case 1:
                pRFTransmitter->setTransmitPower(m_txPower);
                pRFReceiver->setReceiverSensitivity(m_rxSensativity);
                break;
            case 2:
                pRFTransmitter->setTransmitPower(m_txPower);
                pRFReceiver->setReceiverSensitivity(m_rxSensativity);
...
//    time_t tm = time(NULL);
      ...
      string filename = SSTR(tm) + ".json";

       string json = "{\"readerId\": \"";
       json.append(macAddress).append("\", \"tagCount\": ");
       json.append(SSTR(nEntry)).append(", \"tags\": [");
      ...
      for(
          ...
          Cur++)
      {
          json.append("{\"readId\": ").append(SSTR(cnt)).append(", ");
          
          if(++cnt !=nEntry)
          {
            json.append(printOneTagReportData(*Cur)).append("},");
          }
          else
          {
            json.append(printOneTagReportData(*Cur)).append("}]}");
  ...
  CTagReportData *              pTagReportData)
{
    ...
    llrp_u64_t timestamp = 
        pTagReportData->getFirstSeenTimestampUTC()->getMicroseconds() / 1000000;
...
*/
        string json = "\"epc\": \"";
        json.append(epc).append("\", ");
        json.append("\"timestamp\": ").append(SSTR(timestamp)).append(", ");
        json.append("\"antenna\": ").append(SSTR(antennaId));
